...
基于 **FastAPI + Sisyphus Orchestrator + 13 个领域 Agent + 进程内 Event Bus** 的供应链协同管理课程项目。
...
| `monitoring_agent` | 健康检查、数据库与 LLM 状态 | `/api/health`、`/api/health/db`、`/api/llm/status`、`/api/example/*` |
| `analysis_agent` | LLM 自动预警分析与补货风险分析 | `/api/analysis/*`；订阅库存变动事件驱动 LLM 分析 |

Sisyphus 注册状态可通过 `GET /api/system/agents` 查看，正常情况下返回 13 个 Agent。
...
│   │   └── analysis_agent/
│   ├── kernel/
│   │   ├── common/              # 配置、数据库、认证、事件总线、共享查询、LLM 服务
│   │   └── sisyphus/            # Agent 注册、路由挂载与工作流编排
│   ├── main.py                  # FastAPI 应用入口
│   ├── requirements.txt         # 后端运行依赖
│   └── .env.example             # 环境变量示例
├── frontend/                    # 后端直接托管的静态前端（HTML + JS + ECharts），无构建步骤
│   ├── index.html
│   ├── app.js
│   ├── api.js
│   └── style.css
...
这些脚本必须从**项目根目录**运行，不可从 `backend/` 目录运行：
...
- `generate_example_data.py` 重新生成 `example/` 下的课程演示 JSON。
...
看到以下信息说明 13 个 Agent 已正常注册：

```text
[System] All 13 agents registered. Sisyphus orchestrator ready.
```
...
-> AnalysisAgent 触发 LLM 预警分析与补货风险分析
-> 请求事务统一提交
```
...
| 调用方 | 方法 | 用途 |
|---|---|---|
| `recommendation_agent` | `enhance_reason()` | 生成自然语言补货理由文本 + 风险等级 |
| `supplier_agent` | `evaluate_supplier()` | 供应商评分（0-100） |
| `analysis_agent` | `analyze_inventory_risk()` | 库存预警分类 |
| `analysis_agent` | `evaluate_restock_risk()` | 补货风险分类 |

补货数量和风险等级由规则逻辑计算，LLM 只用于增强推荐理由文本和分析文本，不决定核心库存业务。
...
未配置外部模型时，系统自动降级到规则引擎，核心业务仍可正常运行。

LLM 状态可通过 `GET /api/llm/status` 查看。

## 测试

当前多智能体框架已建立 pytest 测试目录。运行测试：
...
- `conftest.py` 自动将 `DATABASE_URL` 覆写为 SQLite，测试使用独立数据库文件 `backend/schema/test_suite.db`。
- 测试会话会自动重建数据库并加载种子数据。
...
- 当前没有 Alembic 数据库迁移。
- `Dockerfile` 和 `docker-compose.yml` 已提供，但目前本地脚本启动流程是已验证的主要运行方式。
