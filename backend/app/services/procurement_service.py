"""采购与入库业务逻辑 — 从 agents/procurement_agent/handler.py 迁移。"""
...
def create_purchase_order(db: Session, data: dict):
    ...
    for i in items_data:
        db.add(PurchaseOrderItem(
            ...
            subtotal_amount=i["purchase_quantity"] * i["purchase_price"],
            **i,
        ))
    db.flush()
    return order
...
def confirm_purchase_order(db: Session, oid: int):
    ...
    if o.status != "pending":
        raise BusinessException("only pending can be confirmed")
    o.status = "confirmed"
    db.flush()
    return o
...
def complete_inbound(db: Session, iid: int):
    ...
    if o.status != "pending":
        raise BusinessException("only pending can be completed")
    o.status = "completed"
    items = list(db.scalars(select(InboundItem).where(InboundItem.inbound_order_id == iid)))
    db.flush()
    event_bus.publish(Event(type=EVENT_INBOUND_COMPLETED, source="procurement_service", data={
