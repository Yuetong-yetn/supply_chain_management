"""供应商业务逻辑 — 从 agents/supplier_agent/handler.py 迁移。"""
...
EVENT_SUPPLIER_SCORED = "supplier.scored"


def list_suppliers(db: Session, page=1, page_size=20, keyword=None):
    q = select(Supplier)
    if keyword:
        q = q.where(Supplier.name.contains(keyword))
...
def recalculate_scores(db: Session, use_llm: bool = True):
    ...
    provider = get_llm_provider() if use_llm else None
    ...
    event_bus.publish(Event(
        type=EVENT_SUPPLIER_SCORED, source="supplier_service",
...
def get_supplier_score(db: Session, sid: int):
    return db.scalar(
        select(SupplierScoreSnapshot).where(SupplierScoreSnapshot.supplier_id == sid)
...
def bind_supplier_product(db: Session, sid: int, data: dict) -> SupplierProduct:
    ...
    sp = SupplierProduct(
        supplier_id=sid, product_id=int(product_id),
