"""库存业务逻辑 — 从 agents/inventory_agent/handler.py 迁移。"""
...
EVENT_STOCK_INCREASED = "inventory.stock.increased"
EVENT_STOCK_DECREASED = "inventory.stock.decreased"
...
def increase_stock(db: Session, product_id: int, location_type: str, quantity: int,
    ...
    db.flush()
    event_bus.publish(Event(type=EVENT_STOCK_INCREASED, source="inventory_service", data={
...
def decrease_stock(db: Session, product_id: int, location_type: str, quantity: int,
    ...
    inv.last_updated_at = datetime.now(timezone.utc)
    db.flush()
    event_bus.publish(Event(type=EVENT_STOCK_DECREASED, source="inventory_service", data={
        "product_id": product_id, "location_type": location_type, "warehouse_id": warehouse_id,
        "store_id": store_id, "change_quantity": -quantity, "before": before, "after": inv.current_quantity,
        "operator_id": operator_id, "transaction_type": transaction_type,
        "related_doc_type": related_doc_type, "related_doc_id": related_doc_id,
...
def get_warnings(db: Session):
    ...
    for inv in inventories:
        wt = None
        if inv.current_quantity <= inv.safety_stock * 0.5:
            wt = "critical_stockout"
        elif inv.current_quantity <= inv.safety_stock:
            wt = "stockout"
        elif inv.current_quantity >= max(inv.max_stock, inv.safety_stock * 4):
            wt = "overstock"
        if wt:
            warnings.append({
...
def adjust_stock(db: Session, product_id: int, location_type: str, new_quantity: int,
    ...
    inv.last_updated_at = datetime.now(timezone.utc)
    db.flush()
    event_type = EVENT_STOCK_INCREASED if new_quantity >= before else EVENT_STOCK_DECREASED
    event_bus.publish(Event(type=event_type, source="inventory_service", data={}))
    return inv
