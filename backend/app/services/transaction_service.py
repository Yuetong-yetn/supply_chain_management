...
def list_transactions(db: Session, page=1, page_size=20):
    ...
    items = list(db.scalars(
        select(StockTransaction).order_by(StockTransaction.id.desc())
...
def get_product_transactions(db: Session, pid: int):
    return list(db.scalars(
        select(StockTransaction).where(StockTransaction.product_id == pid)
...
def get_doc_transactions(db: Session, doc_type: str, doc_id: int):
    return list(db.scalars(
        select(StockTransaction).where(
            StockTransaction.related_doc_type == doc_type,
            StockTransaction.related_doc_id == doc_id,
        )
    ))
...
def trace_product(db: Session, pid: int):
    ...
    return [{
        "transaction_no": t.transaction_no, "transaction_type": t.transaction_type,
        "change_quantity": t.change_quantity, "transaction_time": str(t.transaction_time),
    } for t in txs]
