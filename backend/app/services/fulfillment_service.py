"""补货与出库业务逻辑 — 从 agents/fulfillment_agent/handler.py 迁移。"""
...
EVENT_REPLENISHMENT_CONVERTED = "fulfillment.replenishment.converted"
EVENT_OUTBOUND_SHIPPED = "fulfillment.outbound.shipped"
EVENT_OUTBOUND_SIGNED = "fulfillment.outbound.signed"
...
def approve_request(db: Session, rid: int, audited_by: int):
    ...
    if r.audit_status != "pending":
        raise BusinessException("only pending can be approved")
    r.audit_status = "approved"
    r.audited_by = audited_by
    r.audit_time = datetime.now(timezone.utc)
    db.flush()
    return r
...
def reject_request(db: Session, rid: int, audited_by: int):
    ...
    if not r:
        raise BusinessException("not found", 404)
    if r.audit_status != "pending":
        raise BusinessException("only pending can be rejected")
...
def convert_to_outbound(db: Session, rid: int, source_warehouse_id: int | None = None, handled_by: int = 1):
    ...
    if not r:
        raise BusinessException("not found", 404)
    if r.audit_status != "approved":
        raise BusinessException("only approved can convert")
    ...
    o = OutboundOrder(
        outbound_no=generate_no("OUT", db), source_warehouse_id=source_warehouse_id,
...
def ship_order(db: Session, oid: int):
    ...
    if not o:
        raise BusinessException("not found", 404)
    if o.status != "pending":
        raise BusinessException("only pending can ship")
    o.status = "shipped"
    db.flush()
    items = list(db.scalars(select(OutboundItem).where(OutboundItem.outbound_order_id == oid)))
    event_bus.publish(Event(type=EVENT_OUTBOUND_SHIPPED, source="fulfillment_service", data={
...
def sign_order(db: Session, oid: int):
    ...
    if not o:
        raise BusinessException("not found", 404)
    if o.status != "shipped":
        raise BusinessException("only shipped can sign")
    o.status = "signed"
    db.flush()
    items = list(db.scalars(select(OutboundItem).where(OutboundItem.outbound_order_id == oid)))
    event_bus.publish(Event(type=EVENT_OUTBOUND_SIGNED, source="fulfillment_service", data={
        "order_id": o.id, "warehouse_id": o.source_warehouse_id, "store_id": o.target_store_id,
        "items": [{"product_id": i.product_id, "quantity": i.quantity} for i in items],
        "handled_by": o.handled_by,
        "_db": db,
...
def cancel_order(db: Session, oid: int):
    ...
    if not o:
        raise BusinessException("not found", 404)
    if o.status in ("shipped", "signed"):
        raise BusinessException("cannot cancel shipped/signed order")
    o.status = "cancelled"
    db.flush()
    return o
