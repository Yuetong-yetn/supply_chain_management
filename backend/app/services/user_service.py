"""用户认证业务逻辑 — 从 agents/user_agent/handler.py 迁移。"""
...
def get_user_profile(db: Session, user_id: int) -> dict:
    ...
    return {
        ...
        "role": user.role, "location_type": user.location_type,
...
def get_identity_by_employee_no(db: Session, employee_no: str) -> dict:
    ...
    from app.services.location_service import get_location_name

    user = db.scalar(select(User).where(User.employee_no == employee_no))
    if not user:
        raise BusinessException("未找到该工号对应的用户")
    location_name = get_location_name(db, user.location_type, user.warehouse_id, user.store_id)
    return {
